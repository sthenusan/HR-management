...
<div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Remaining Leave <b>History</b></h2>
                    </div>
                    <div class="col-sm-6">
    ...
    <table class="table table-striped table-hover">
                <thead>
                    <tr>
                         ...
                         <th>Leave Type</th>
                        <th>Allowed_Leave</th>
                        <th>Sum_Of_Taken_Leave</th>
                         <th>Remaining_Leave</th>
...
$leave_id='';
        $query11 = "SELECT leave_id  FROM leave_type ";
        $result=mysqli_query($db, $query11);
        $applicationArray=array();
        while($row=mysqli_fetch_assoc($result)){
            $applicationArray[]=$row;
        ...
        foreach ($applicationArray as $application) {
            ...
            $leave_type='';
        $query11 = "SELECT leave_type  FROM leave_type WHERE leave_id='$leave_id';";
        $result=mysqli_query($db, $query11);
        $applicationArray=array();
        while($row=mysqli_fetch_assoc($result)){
            $applicationArray[]=$row;
...
$allowed_leave='';
 ...
 if ($gender=='m' and $leave_type=='maternity'){
     $remaining_leave="Employee cannot apply for maternity leave";
 }
 else if ($allowed_leave>$days){
     $remaining_leave=$allowed_leave-$days;
 }

 else{
     $remaining_leave='Employee have taken the allow leave limit';
 }
